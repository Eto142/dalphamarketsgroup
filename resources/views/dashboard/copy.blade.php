...
<script src="https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/js/bootstrap.min.js"></script>
            ...
            <div class="col-lg-4">
                <div class="text-center card-box">
                    ...
                    <div class="member-card pt-2 pb-2" style="border:10px;">
                       ...
                       <form id="copyTraderForm" action="{{ route('copyTrader') }}" method="POST" onsubmit="return handleFormSubmit(event, '{{ $trader->id }}')">
    @csrf
    <div>
    ...
    <button type="submit" id="copyButton{{ $trader->id }}"
...
<script>
    function handleFormSubmit(event, traderId) {
        // Call your custom function to handle the copy functionality
        toggleCopyText(traderId);

        // Proceed with form submission
        return true; // Ensure the form is submitted after running the custom function
    }
</script>

<script>
function toggleCopyText(traderId) {
    const nameToCopy = document.querySelector(`input[name='copy_name']`).value;

    // Copy to clipboard
    navigator.clipboard.writeText(nameToCopy).then(() => {
        alert('Copied: ' + nameToCopy); // Feedback after copy

        // Send AJAX request to save the copied name
        fetch('/copy-trader', {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({ trader_name: nameToCopy })
        })
        ...
        .catch(err => {
            console.error('Error saving trader name:', err);
        });
    }).catch(err => {
