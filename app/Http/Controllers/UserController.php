...
use App\Models\Debitreferral;
use App\Models\Debitdeposit;
...
try {
    ...
    try {
        ...
        if (isset($raw['data']['priceUsd'])) {
            $data = ['bitcoin' => ['usd' => (float)$raw['data']['priceUsd']]];
        } else {
...
$price = $data['bitcoin']['usd'];
                    ...
                    $data['deposit'] = $data['userdeposit'] - $data['debitdeposit'];
                    ...
                    $data['userreferral'] = Refferal::where('user_id', Auth::user()->id)->sum('amount');
                    $data['debitreferral'] = Debitreferral::where('user_id', Auth::user()->id)->sum('amount');
                    $data['referral'] = $data['userreferral'] - $data['debitreferral'];
...
$price = $data['bitcoin']['usd'];
    ...
    $credit = Transaction::where('user_id', Auth::id())->where('status', '1')->sum('credit');
    $debit = Transaction::where('user_id', Auth::id())->where('status', '1')->sum('debit');
    $user_balance = $credit - $debit;
    $btc_balance = $user_balance / $price;
    $traders = Traders::all();

if ($user_balance == 0) {
    return redirect()->route('home')
        ->with('show_deposit_modal', true)
        ->with('notice', 'Your balance is 0. Fund Account to Copy Trader.');
}
    ...
    return view('dashboard.copy', compact('price', 'user_balance', 'btc_balance', 'traders'));
}
...
// $data['amount'] = $amount;
// $data['btc_amount'] = $data['amount']  / $btc_price;
// $data['eth_amount'] = $data['amount']  / $eth_price;
...
//     $btc_price = $prices['result']['XXBTZUSD']['c'][0]; // Latest BTC price
//     $eth_price = $prices['result']['XETHZUSD']['c'][0]; // Latest ETH price
...
//     $data['credit'] = Transaction::where('user_id', Auth::id())->where('status', '1')->sum('credit');
//     $data['debit'] = Transaction::where('user_id', Auth::id())->where('status', '1')->sum('debit');
//     $data['user_balance'] = $data['credit'] - $data['debit'];
//     $data['btc_balance'] = $data['user_balance'] / $btc_price;
...
//     $amount = $request->input('amount');
//     $data['amount'] = $amount;
//     $data['btc_amount'] = $amount / $btc_price;
//     $data['eth_amount'] = $amount / $eth_price;
...
public function getDeposit(Request $request)
{
    ...
    try {
        ...
        cache()->put('btc_price', $btc_price, now()->addMinutes(10));
        cache()->put('eth_price', $eth_price, now()->addMinutes(10));

    } catch (\Exception $e) {

        try {
            // 🔹 BACKUP API (CoinCap)
            $response = $client->get('https://api.coincap.io/v2/assets', [
                'timeout' => 5,
            ]);

            $assets = json_decode($response->getBody(), true)['data'];

            $btc_price = collect($assets)->firstWhere('id', 'bitcoin')['priceUsd'];
            $eth_price = collect($assets)->firstWhere('id', 'ethereum')['priceUsd'];
        } catch (\Exception $e2) {
            // 🔹 FALLBACK: use cached or static prices if both APIs are unreachable
            $btc_price = cache()->get('btc_price', 60000);
            $eth_price = cache()->get('eth_price', 3000);
        }
    }
    ...
    $data['btc_balance'] = $user_balance / $btc_price;
...
public function store(Request $request)
{
    ...
    $request->validate([
        'copy_name' => 'required|string|max:255',
    ]);

    // Create a new Copy object
    $copy = new Copy;
    $copy->user_id = Auth::id();  // Use authenticated user's ID
    $copy->trader_name = $request->copy_name;  // Assign trader name from request
...
public function ConfirmPassword(Request $request)
{
     ...
     public function SocialMediaUpdate(Request $request)
    {
        ...
        $user->update();
        return back()->with('status', 'Social Media Handle Updated');
    }
...
// Fetch BTC & ETH prices in USD from CoinGecko
$response = $client->get('https://api.coingecko.com/api/v3/simple/price', [
    'query' => [
        'ids' => 'bitcoin,ethereum',
        'vs_currencies' => 'usd',
...
$price = $data['bitcoin']['usd'];
                ...
                return view('dashboard.success-step2', compact('data'));
