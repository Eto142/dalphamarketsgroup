...
class UserManagementController extends Controller
{
    ...
    public function userProfile($id)
    {
        ...
        $copies      = DB::table('copies')->where('user_id', $id)->orderBy('id', 'desc')->get();
        ...
        // sum transactions
        $userDeposit      = DB::table('deposits')->where('user_id', $id)->where('status', '1')->sum('amount');
        $debitDeposit      = DB::table('debitdeposits')->where('user_id', $id)->sum('amount');
        $totalDeposit      =  $userDeposit  -  $debitDeposit;
        ...
        $userBonus      = DB::table('refferals')->where('user_id', $id)->sum('amount');
        $debitBonus      = DB::table('debitreferrals')->where('user_id', $id)->sum('amount');
         $totalBonus      =  $userBonus  -  $debitBonus;
        ...
        return view('manager.user', $data, compact('userProfile', 'userProfit', 'totalBalance', 'totalProfit', 'totalDeposit', 'totalBonus', 'totalWithdrawal', 'kyc', 'copies'));
    }
...
public function DeclineWithdrawal(Request $request, $id)
{
 ...
 public function debitUserDeposit(Request $request)
    {
        // $validate->validate($request,[
        //     'subject' => 'required',
        //     'message' => 'required'
        ...
        $transaction->status = 1;
        $transaction->save();
        return redirect()->back()->with('message', 'User Deposit Debited Successfully');
    }




public function debitUserReferral(Request $request)
    {
        // $validate->validate($request,[
        //     'subject' => 'required',
        //     'message' => 'required'
        ...
        $transaction->status = 1;
        $transaction->save();
        return redirect()->back()->with('message', 'User Bonus Debited Successfully');
    }
